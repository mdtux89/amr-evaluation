...
sed 's/ ( / <OP> /g' $1.sentences | sed 's/ ) / <CP> /g' | sed 's/( /<OP> /g' | sed 's/ )/ <CP>/g' > $1.sentences.nopars
echo "Running CoreNLP.."
java -mx6g -cp "$CORENLP/*" edu.stanford.nlp.pipeline.StanfordCoreNLP -props "../corenlp.properties" -file "$1.sentences.nopars" --outputFormat text --outputDirectory "$workdir"
